...
. Modify the resulting SVG manually (using any SVG editor) to improve the key shapes, if desired. In my case only the ENTER key needed this. Make sure you keep the `x-key-name` attributes in the SVG file.
